...
CP = {'omg_m':0.24, 'hub':0.73, 'sigma8':0.80, 'rho_c':6.6*1e+11, 'k_min': 1.0e-4, 'kmax_linear':600.0, 'kmax_halo':20.0, 'num_k': 600, 'r_min':0.01, 'r_max':99, 'num_r': 200}
...
def sigm(pok_file):
	...
	pk_interp = interp1d(pok_dat[:,0], pok_dat[:,1])    # linear P(k)
...
def ZHfunc(sig_arr, detc_arr):
	...
	step = 0.02
	sigm2_new = np.arange(sig2new_min, sig2new_max, step)
	num_sig2new = len(sigm2_new)
	detcsig2_interp = interp1d(sigm2_array, detc_arr)
...
# g2 function  g2[i,j] = g_2(S_i, S_j - Delta S/2)
        ...
        barsigm2_interp = interp1d(sigm2_new,bar_array)
...
# f(S) function: F = (I - M)^(-1)*G 
	...
	dSdm_array = 2.0*dlsdlm_arr/mass_arr*sigm2_array
	dSdm_interp = interp1d(sigm2_array, dSdm_array)
	dSdmZH_array = dSdm_interp(sigm2_new)
        dndlnm_temp = CP['rho_m']*ffunc*abs(dSdmZH_array)
	dndlnm_iterp = interp1d(sigm2_new, dndlnm_temp)
	dndlnm = np.zeros(CP['num_r'])
	dndlnm[0] = dndlnm_temp[CP['num_r']-1]
	dndlnm[1:CP['num_r']-1] = dndlnm_iterp(sigm2_array[1:CP['num_r']-1])
	dndlnm[CP['num_r']-1] = dndlnm_temp[0]
...
def ZHfucplbias(sig_arr, detc_arr):
	deriv_step = 0.0001
        ...
        deriv_nuFu = -np.log(ZHfunc(sig_arr, detc_arr+2.0*step_arr))
        deriv_nuFu = deriv_nuFu + 8.0*np.log(ZHfunc(sig_arr, detc_arr+step_arr))
        deriv_nuFu = deriv_nuFu - 8.0*np.log(ZHfunc(sig_arr, detc_arr-step_arr))
        deriv_nuFu = deriv_nuFu + np.log(ZHfunc(sig_arr, detc_arr-2.0*step_arr))
        deriv_nuFu = deriv_nuFu / (12.0*deriv_step)
	Bias = 1.0 - deriv_nuFu
	dndlnm = ZHfunc(sig_arr, detc_arr)
...
HS = {}
HS['dtac_file'] = './halocritemp.txt'    #pre-calcuated delta_c file
HS['pok_file'] = './hsn1fR0m1m5_matterpower.dat'  #linear Hu-Sawicki p(k) from EFTCAMB
...
hsdetvir_iterp = interp1d(HS['dtac_dat'][:,1],  Delvir_array)

HS['sigma'] = sigm(HS['pok_file'])
hsdetc_iterp = interp1d(HS['dtac_dat'][:,1],  HS['dtac_dat'][:,6])
...
LCDM = {}
LCDM['pok_file'] = './Planck2015_matterpower.dat'  #linear Hu-Sawicki p(k) from EFTCAMB
...
class Halo(object):
	...
	def pok(self):
		...
		bias_interp = interp1d(mass_arr, self.bias)
		nlnm_interp = interp1d(mass_arr, self.ndlnm)
		pk_dat = np.loadtxt(self.name['pok_file'])
		pk_interp = interp1d(pk_dat[:, 0], pk_dat[:, 1])
		plk_arr = pk_interp(self.karr)     
		
		print "linear matter spectrum: ", plk_arr
		...
		for i in range(CP['num_k']):
		        rho_interp = interp1d(mass_arr, self.NFW[i, :])
...
LCDM_Halo = Halo(LCDM, "ST")
HS_ZHHalo = Halo(HS, "ZH")
HS_STHalo = Halo(HS, "ST")

Halos = {"HS_ST":HS_STHalo, "HS_ZH":HS_ZHHalo, "LCDM_ST":LCDM_Halo}

for Halo in Halos.keys():
	pok_tmp = Halos[Halo].pok()
	file_tmp = Halo + '_pok_array.txt'
